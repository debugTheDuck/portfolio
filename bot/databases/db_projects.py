...
async def add_project(data):
    with sqlite3.connect('../../projects.db') as db:
        ...
        unique_hash = create_hash(data["project_type"], data["project_name"])
        project_img = json.dumps(data["project_img"])
        project_description = json.dumps(data["project_description"])
        project_content_counter = json.dumps(data["counter"])
        project_content = json.dumps([data[f"content{i+1}"] for i in range(project_content_counter)])

        print([data[f"content{i+1}"] for i in range(project_content_counter)])
...
async def create_hash(project_type, project_name):
    ...
    while is_unique == False:
        ...
        unique_hash = generate_hash(project_name)
